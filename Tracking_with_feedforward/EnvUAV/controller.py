...
class Controller:
    def __init__(self, path, time_step=0.01):
        ...
        self.last_ang_v_d_tilde = self.current_ang_v_d_tilde = None
    ...
    def step(self, time_index, target, pos, vel, last_R, current_R, ang_v):
        ...
        self.last_R_tilde = self.current_R_tilde = np.zeros(shape=[3,3])
        self.last_ang_v_d_tilde = self.current_ang_v_d_tilde = np.zeros(shape=[3])
        ...
        roll_tilde = np.arcsin(
            (np.sin(yaw) * fx_tilde - np.cos(yaw) * fy_tilde) / np.linalg.norm([fx_tilde, fy_tilde, fz_tilde]))
        pitch_tilde = np.arctan((np.cos(yaw) * fx_tilde + np.sin(yaw) * fy_tilde / fz_tilde))
        combined_quaternion = p.getQuaternionFromEuler([roll_tilde, pitch_tilde, yaw])
        self.current_R_tilde = np.reshape(p.getMatrixFromQuaternion(combined_quaternion), [3,3])
        ...
        ang_acc_d_tilde = (self.current_ang_v_d_tilde - self.last_ang_v_d_tilde) / self.time_step
        ang_acc_d_tilde = np.matmul(current_R, np.matmul(self.current_R_tilde.T, ang_acc_d_tilde))

        with torch.no_grad():
            tau_roll = (self.actor_ang(torch.tensor(sroll, dtype=torch.float)).item() + ang_acc_d_tilde[
                0]) * self.ang_acc_bound * self.J_xx
            tau_pitch = (self.actor_ang(torch.tensor(spitch, dtype=torch.float)).item() + ang_acc_d_tilde[
                1]) * self.ang_acc_bound * self.J_yy
            tau_yaw = (self.actor_ang(torch.tensor(syaw, dtype=torch.float)).item() + ang_acc_d_tilde[
                2]) * self.ang_acc_bound * self.J_zz
